...
exports.ContextIsolationManager = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
class ContextIsolationManager {
    constructor() {
        this.contextStores = new Map();
        this.compressionRatio = 0.1; // Compress to 10% of original size
    }
    async createIsolatedContext(agentId, stage, userRequirements) {
    ...
    async gatherRelevantFiles(stage) {
        const relevantFiles = [];
        try {
            ...
            const miniPromptPath = path.join(workspaceFolder.uri.fsPath, 'public', 'playbook', 'mini-prompts', `${stage.miniPromptPath}.md`);
...
`;
    ...
    calculateContextSize(taskContext, relevantFiles, handoffSummary) {
    ...
    async storeContext(agentId, context) {
        const contextStore = {
    ...
    async compressHandoff(fullContext, targetTokens) {
        ...
        try {
            ...
            const compressedContext = this.createCompressedContext(keyDecisions, outputs, nextSteps, targetTokens);
            return {
    ...
    extractOutputs(context) {
        if (context.outputs && context.outputs instanceof Map) {
            return Array.from(context.outputs.keys()).slice(0, 10); // Limit to 10 outputs
        }
        return ['Context outputs extracted'];
    }
    identifyNextSteps(context) {
    ...
    createCompressedContext(keyDecisions, outputs, nextSteps, targetTokens) {
...
`;
    ...
    async cleanupExpiredContexts(maxAge = 3600000) {
...
exports.ContextIsolationManager = ContextIsolationManager;
//# sourceMappingURL=context-isolation-manager.js.map