...
/**
 * Data structure for skill display
 ...
 */
export interface SkillCardVisibility {
  showActive?: boolean;
  showPublic?: boolean;
  showShare?: boolean;
  showRate?: boolean;
 ...
 */
export interface SkillCardHandlers {
  onCardClick?: () => void;
...
export interface SkillDiscoveryCardProps {
  skill: SkillCardData;
  state: SkillCardState;
  visibility: SkillCardVisibility;
  handlers: SkillCardHandlers;
  testId?: string;
}

/**
 * SkillDiscoveryCard - Pure UI Component
 *
 * A presentation-only card component for displaying skill information.
 * All business logic (API calls, state management) should be handled by
 * a parent container/widget component.
 *
 * Uses cyan accent color (like workflows, not pink like prompts).
 *
 ...
 */
export function SkillDiscoveryCard({
  skill,
  state,
  visibility,
  handlers,
  ...
  const cardTestId = testId || `skill-${skill.id}`;
        ...
        <div className="p-4 sm:p-6 flex-1 flex flex-col">
          <div className="flex items-start justify-between gap-2 mb-2 sm:mb-3 pr-8">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-lg sm:text-xl font-bold text-cyan-100 hover:text-cyan-400 transition-colors line-clamp-2">
                {skill.name}
              </h3>
            </div>
            {skill.rating &&
              skill.rating.average !== null &&
          ...
          {skill.tags && skill.tags.length > 0 && (
            <div className="mb-2">
              <TagBadgeList tags={skill.tags} />
            </div>
          )}

          {skill.models && skill.models.length > 0 && (
            <div className="mb-2 sm:mb-3">
              <ModelBadgeList models={skill.models} />
            </div>
          )}
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-cyan-100/60">
            ...
            <span
              ...
              title={`${skill.attachmentCount} attachments`}
            >
              <Paperclip className="w-4 h-4" />
              {skill.attachmentCount}{" "}
              <span className="hidden sm:inline">
                {skill.attachmentCount === 1 ? "file" : "files"}
              </span>
            </span>
            {skill.referencesCount !== undefined && (
              <span
                className="flex items-center gap-1"
                title={`${skill.referencesCount} users added this to their library`}
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
